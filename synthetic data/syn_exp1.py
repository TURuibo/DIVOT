...
def test(batches,key,step_sz,exp,nrep):
    df_batch = batches[0]
    batch_sz,_ = df_batch.shape
    theta_H =0.2
    params = theta_H
...
def main():
	...
	for seed in range(num_exp):
		...
		# The optimization parameters are fixed.
	...
	print('\nnsamples:',nsamples,", batch size:",resolution,", npos:",npos, ', accuracy:', res/num_exp, ', theta:', np.mean(theta_est),', standard deviation of estimated theta:', np.std(theta_est))
